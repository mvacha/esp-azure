...
extern const uint8_t leaf_cert_pem_start[] asm("_binary_device_cert_pem_start");
...
char* custom_hsm_get_certificate(HSM_CLIENT_HANDLE handle)
{
    ...
    else
    {
        ...
        //return one dummy byte, so iothub sdk has something to free
...
char* custom_hsm_get_key(HSM_CLIENT_HANDLE handle)
{
    ...
    printf("Returning dummy key instead of private key (HSM).");
...
char* custom_hsm_get_common_name(HSM_CLIENT_HANDLE handle)
{
    ...
    else
    {
        ...
        printf("Returning common name:%s.", result);
    }
    return result;
}
